...
function CalendarView({ trades, accounts, onNewTrade, onEditTrade, onDeleteTrade }) {
  ...
  // Métricas reais do mês, calculadas a partir dos trades cadastrados
  const monthWins = monthTrades.filter((t) => Number(t.pnl) > 0);
  const monthLosses = monthTrades.filter((t) => Number(t.pnl) < 0);
  const monthWinRate = monthTrades.length ? Math.round((monthWins.length / monthTrades.length) * 100) : 0;
  const grossWin = monthWins.reduce((s, t) => s + Number(t.pnl), 0);
  const grossLoss = Math.abs(monthLosses.reduce((s, t) => s + Number(t.pnl), 0));
  const avgWin = monthWins.length ? grossWin / monthWins.length : 0;
  const avgLoss = monthLosses.length ? grossLoss / monthLosses.length : 0;
  const monthRR = avgLoss > 0 ? (avgWin / avgLoss).toFixed(2) : (avgWin > 0 ? "∞" : "—");

  const sortedMonthTrades = [...monthTrades].sort((a, b) => a.trade_date.localeCompare(b.trade_date));
  let runningEq = 0;
  const monthEquityCurve = sortedMonthTrades.map((t) => {
    runningEq += Number(t.pnl);
    return { d: t.trade_date.slice(8, 10), eq: runningEq };
  });

  const dailyPerf = Object.keys(calendarPnL)
    .map(Number)
    .sort((a, b) => a - b)
    .map((d) => ({ day: d, pnl: calendarPnL[d] }));
  const maxAbsDaily = Math.max(1, ...dailyPerf.map((d) => Math.abs(d.pnl)));
...
@media (max-width:860px){
  .tf-wa-float{ width:48px; height:48px; bottom:calc(16px + env(safe-area-inset-bottom)); right:16px; }
}
